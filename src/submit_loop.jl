function submit_scripts(path_txt; verbose::Bool=true, priority="")
    ...
    submit_scripts!(dict_jobs, path_jobdb=path_jobdb, verbose=verbose, priority=priority)
end

function submit_scripts!(dict_jobs::OrderedDict{String, SLURMJob};
    path_jobdb::String, verbose::Bool=true, priority="")
    list_key_submit = []
    for (k_, v_) = dict_jobs
        if !v_.submitted
    ...
    for k_ = list_key_submit
        ...
        while !(q_submit_ok) && (n_attempt < MAX_ATTEMPT)
            ...
            try
                jobid = squeue_submit_sbatch(job.path_sh, priority=priority)
                job.jobid = jobid
                job.submitted = true
                q_submit_ok = true
                verbose &&
